...
func (s *WechatBannerRouter) InitWechatBannerRouter(Router *gin.RouterGroup, PublicRouter *gin.RouterGroup) {
	// 微信轮播图API不需要认证，放在PublicRouter中
	wechatBannerPublicRouter := PublicRouter.Group("api")
	{
		wechatBannerPublicRouter.GET("wxBanners", wechatBannerApi.WxGetBanners) // 微信小程序轮播图
	}
}